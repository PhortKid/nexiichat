...
async function sendMetatemplet(
  ...
  try {
    console.log("Sending template with body:", JSON.stringify(body, null, 2));
...
async function processImage({ sourcePath, savingPath, width, height }) {
  ...
  if (ext === "ico") {
    ...
    fs.writeFileSync(outputPath, Buffer.concat([header, pngBuffer]));
  } else if (ext === "png") {
  ...
  console.log(`✅ Saved: ${outputPath} [${width}×${height}] .${ext}`);
  return outputPath;
}
