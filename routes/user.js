...
router.post("/update_profile", validateUser, async (req, res) => {
  try {
    ...
    let mobile = mobileFromBody; // Use let instead of const
...
router.post("/update_fcm_token", validateUser, async (req, res) => {
  try {
    ...
    console.error("❌ USER FCM update error:", err);
    res.status(500).json({
      success: false,
      msg: "Something went wrong",
      err: err.message,
...
router.post("/exchange_embed_token", validateUser, async (req, res) => {
  try {
    ...
    console.log("📥 /exchange_embed_token received:", {
    ...
    console.log("🔄 exchangeEmbedToken result:", {
    ...
    if (genToken?.success) {
      ...
      if (is_coexistence) {
        console.log("🔄 Starting coexistence sync...");
        setTimeout(async () => {
          try {
            await syncContacts({
              token: access_token,
            ...
            console.log("✅ Coexistence sync initiated");
          } catch (syncErr) {
    ...
    console.error("❌ /exchange_embed_token error:", err);
    res.json({ success: false, msg: "Something went wrong", err: err.message });
  }
});
...
router.post("/try_js", async (req, res) => {
  ...
  try {
    ...
    // ✅ ADD THIS: Provide helpful message when no return
    res.json({
      success: true,
      data: result !== undefined ? result : null, // Convert undefined to null
      logs: logs,
      warning:
